...
- "I want a customer feedback form with a 5-star rating and a comments box"
...
Cloudflare includes database and dashboard with one command. Vercel + Postgres needs a one-time DB setup — see [setup-vercel-postgres.md](docs/setup-vercel-postgres.md). All options: [docs/deploy-options.md](docs/deploy-options.md).
...
The skill is in `.cursor/skills/formant/`. **Cursor:** Open the repo in Cursor — skill is auto-available. **Claude Code:** Copy the skill to your Claude Code skills directory, or use the root `SKILL.md`. Run from the project directory so `pnpm formant build` works.

Add GitHub topics `claude-skill`, `claude-code`, `anthropic`, `form-builder` for skills directory discoverability (gear icon next to About).
